...
describe 'Dns::Key', type: :define do
  ...
  context 'On a Debian OS' do
    ...
    it {
      is_expected.to contain_exec('get-secret-from-rspec-key')
        ...
        .with_creates('/etc/bind/bind.keys.d/rspec-key.secret')
        .with_require(['Exec[dnssec-keygen-rspec-key]', 'File[/etc/bind/bind.keys.d]', 'File[/tmp/rspec-key-secret.sh]'])
    }
    ...
    ['rspec-key.key-header', 'rspec-key.key-secret', 'rspec-key.key-footer'].each do |fragment|
      it {
        is_expected.to contain_concat__fragment(fragment)
          .with_target('/etc/bind/bind.keys.d/rspec-key.key')
          .with_require(['Exec[get-secret-from-rspec-key]', 'File[/etc/bind/bind.keys.d/rspec-key.secret]'])
      }
    end
  ...
  context 'On a RedHat OS' do
    ...
    ['rspec-key.key-header', 'rspec-key.key-secret', 'rspec-key.key-footer'].each do |fragment|
      it {
        is_expected.to contain_concat__fragment(fragment)
          .with_target('/etc/named/bind.keys.d/rspec-key.key')
          .with_require(['Exec[get-secret-from-rspec-key]', 'File[/etc/named/bind.keys.d/rspec-key.secret]'])
      }
    end
